...
def salvar_setores(lista):
    conn = get_db_connection()
    df = pd.DataFrame({"setor": lista})
    df.to_sql("setores", conn, if_exists="replace", index=False)
    conn.close()
...
def salvar_equipamentos(lista):
    ...
    df.to_sql("equipamentos", conn, if_exists="replace", index=False)
    conn.close()
...
if perfil == "👤 Usuário Comum":
    ...
    with aba_novo:
        ...
        with st.form("novo_chamado", clear_on_submit=True):
            ...
            with col2: 
                eq_options = [nome_equipamento(eq) for eq in st.session_state.equipamentos] if st.session_state.equipamentos else ["Nenhum cadastrado"]
            ...
            if st.form_submit_button("🚀 ENVIAR SOLICITAÇÃO", use_container_width=True, type="primary"):
                if nome and descricao and setor:
                    ...
                    if salvar_dados(st.session_state.chamados):
                        st.success(f"✅ Chamado Nº **{novo_id}** registrado!")
                        st.balloons()
                        st.rerun()
                else:
...
# ====================== MANUTENÇÃO ======================
    ...
    tecnicos_ativos = st.session_state.equipe[st.session_state.equipe["ativo"] == 1]["nome"].tolist() or ["Técnico Não Cadastrado"]
    ativos = [c for c in st.session_state.chamados if c["status"] in ["Aberto", "Em Atendimento"]]
   
    if not ativos:
        st.success("🎉 Todos os chamados foram solucionados!")
    else:
        for cham in sorted(ativos, key=lambda x: (PRIORIDADES.get(x.get("prioridade"), 999), x.get("data_hora_abertura", datetime.min))):
            with st.container(border=True):
                ...
                st.markdown(f"**OS Nº {cham['id']}** — :{cor}[{cham.get('prioridade')}]")
                st.write(f"**Setor:** {cham['setor']} | **Equipamento:** {cham.get('equipamento', 'N/A')}")
                st.write(f"**Solicitante:** {cham['solicitante']}")
                st.write(f"**Descrição:** {cham['descricao']}")
               
                if cham.get("foto_path") and isinstance(cham.get("foto_path"), str) and os.path.exists(cham["foto_path"]):
                    st.image(cham["foto_path"], use_container_width=True)
               
                if cham["status"] == "Aberto":
                    nome_tec = st.selectbox("Selecionar Técnico:", tecnicos_ativos, key=f"t_{cham['id']}")
                    if st.button("🚀 Iniciar Manutenção", key=f"b_in_{cham['id']}", use_container_width=True):
                        if nome_tec and nome_tec != "Técnico Não Cadastrado":
                    ...
                    opcao_sol = st.radio("Foto da Solução:", ["Sem foto", "Tirar foto", "Galeria"], horizontal=True, key=f"sol_{cham['id']}")
                    foto_sol = None
                    if opcao_sol == "Tirar foto":
                        f = st.camera_input("Capturar Solução", key=f"cam_sol_{cham['id']}")
                        if f: foto_sol = comprimir_imagem(f, "solucao")
                    elif opcao_sol == "Galeria":
                        f = st.file_uploader("Selecionar foto", type=["jpg","jpeg","png"], key=f"up_sol_{cham['id']}")
                        if f: foto_sol = comprimir_imagem(f, "solucao")
                   
                    solucao_txt = st.text_area("Descreva a solução *", key=f"txt_{cham['id']}")
                   
                    if st.button("✅ Concluir Chamado", key=f"b_fim_{cham['id']}", type="primary"):
                        if solucao_txt.strip():
...
# ====================== ADMINISTRADOR ======================
    ...
    if not st.session_state.admin_logado:
        with st.form("login_admin"):
            ...
            if st.form_submit_button("Efetuar Login"):
                try:
                    ...
                    senha_hash_correto = st.secrets["admin"]["password_hash"]
                    senha_hash = hashlib.sha256(senha_input.encode()).hexdigest()
                    if usuario_input.strip() == usuario_correto and senha_hash == senha_hash_correto:
                        st.session_state.admin_logado = True
                        st.rerun()
                    else:
                        st.error("❌ Usuário ou senha incorretos.")
                except Exception:
                    st.error("⚠️ Configure o arquivo .streamlit/secrets.toml")
    else:
        ...
        with tab8:
            ...
            com_foto = [c for c in st.session_state.chamados
                       if c.get("foto_path") and isinstance(c.get("foto_path"), str) and os.path.exists(c.get("foto_path"))]
            if com_foto:
                cols = st.columns(3)
...
st.caption("**Sistema v1.0** - By Leandro Coelho")